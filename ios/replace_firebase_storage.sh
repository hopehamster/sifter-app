...
if [ ! -f "${STORAGE_SWIFT_PATH}.original" ]; then
  cp "$STORAGE_SWIFT_PATH" "${STORAGE_SWIFT_PATH}.original"
  echo "Created backup at ${STORAGE_SWIFT_PATH}.original"
fi
...
// Use normal import instead of implementation-only import
...
@objc(FIRStorage) open class Storage: NSObject {
   ...
   * - Returns: A `Storage` instance, configured with the custom `FirebaseApp` and URL.
   */
  ...
  internal init(app: FirebaseApp, bucket: String) {
    ...
    let appCheckInstance = ComponentType<AppCheckInterop>.instance(for: AppCheckInterop.self,
                                                              in: app.container)
    self.appCheck = appCheckInstance ?? nil as! AppCheckInterop
...
# Replace the file
mv fixed_storage.swift "$STORAGE_SWIFT_PATH"
