...
class _ChatDetailsScreenState extends State<ChatDetailsScreen> {
  ...
  Future<void> _joinChat() async {
    ...
    if ((authProvider.user?.isAnonymous ?? true) && !allowAnonymous) {
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: Text('Sign In Required'),
          content: Text('This chat does not allow anonymous users. Please sign in to join.'),
          actions: [
            TextButton(
    ...
    await _db.child('rooms/${widget.roomId}/participants').set(ServerValue.increment(1));
